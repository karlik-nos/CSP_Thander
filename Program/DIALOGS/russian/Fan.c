	...
	switch(Dialog.CurrentNode)
	{
		...
		case "CoolStory":
			...
			switch (rand(20))
			{
				...
				break;
				case 2:
					coolstory = "Предложение убрать все тотемы, которые идут по квесту ТЕНО из продажи у торговцев и оставить их только, как квестовые на своих местах, то есть их нужно находить. Сделать это нужно, чтобы не было очевидных абузов в том виде, какой он есть сейчас, то есть покупать комплекты тотемов, Отдавать их офицерам и так можно сделать сколько угодно. К тому же убрав тотемы из продажи это уберет путаницу у игроков с тем, что нельзя складывать один и тот же тотем в сундук иначе он просто пропадет. Многие новички не знают этих деталей. И это ещё одна причина это сделать.";
