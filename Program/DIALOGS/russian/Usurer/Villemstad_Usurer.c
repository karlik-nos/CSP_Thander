...
//********************** Пиратская линейка, квест №5 ОГЗ ************************
		...
		break;
		case "Step_P5_2":
			dialog.text = "Понятия не имею. Он вольный человек, ходит, где заблагорассудится. А насчет того, что он ещё в Виллемстаде, это вряд ли, здесь для него работы не осталось. Вы же "+ GetSexPhrase("приятель","приятельница") +" Джона, должны понимать, что искать его надо там, где ром льется рекой - в тавернах!";
