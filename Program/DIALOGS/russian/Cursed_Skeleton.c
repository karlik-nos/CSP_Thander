	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			if (npchar.quest.meeting == "0")
			{
				...
				link.l1 = "Вот чёрт, я думал" +GetSexPhrase("","а")+" что здесь никто не живёт. Прошу прощения, я сейчас же уйду.";
			...
			else if (npchar.quest.meeting == "2")
			{
				...
				if(CheckCharacterItem(pchar, pchar.questTemp.Cursed.Item) && CheckAttribute(pchar,"cursed.Skullbought"))
				{
					...
					link.l2 = "Ещё чего. Теперь череп мой. Спасибо за наводку, без тебя я бы его не наш"+GetSexPhrase("ёл","ла")+".";
		...
		break;
		case "meeting_6":
			dialog.text = "Мой собственный старпом! Я считал его своим лучшим другом, а он меня предал, ещё и команду подговорил. Все из-за этого проклятого черепа, будь он не ладен.";
