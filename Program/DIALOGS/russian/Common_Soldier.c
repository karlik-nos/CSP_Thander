...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
						...
						if (sti(rColony.HeroOwn) == true) // наш горожанин
						{
			         		switch (rand(10))
							{
								...
								case 3:
									...
									link.l1 = "Я устал"+ GetSexPhrase("","а") +" от этого нытья! Выпивки вам хватает и на земле! Служи там, где тебя поставил капитан! А то кое-кого придётся вздернуть для примера.";
									link.l1.go = "exit";
								break;

								case 4:
									dialog.text = "Скажу вам по секрету, капитан, новый губернатор - взяточник и казнокрад. Но это, конечно, не моё дело...";
									link.l1 = "Ты прав, корсар: твоё дело - стоять на посту и следить за порядком. Ну, а вздернуть губернатора на нок-рее - это уже занятие для меня. Молодец!";
									link.l1.go = "exit";
								break;
								...
								case 8:
									...
									link.l1 = "Ничего, я свое ещё наверстаю. И вам, ребята, я не завидую.";
									link.l1.go = "exit";
								break;
		...
		else
		{
			...
			dialog.text = RandPhraseSimple("Сдается мне, что это обман... Давай-ка пройдём в комендатуру, "+ GetSexPhrase("голубчик","голубушка") +", там разберёмся...", "Хм, что-то подсказывает мне, что ты не "+ GetSexPhrase("тот","та") +", за кого себя выдаёшь... Немедленно сдайте оружие, " + GetAddress_Form(npchar) + ", и следуйте за мной для дальнейшего разбирательства!");
