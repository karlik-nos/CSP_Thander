...
//>>>>>>>>>>>>========= Разброс диалогов =====================
			...
			if (pchar.questTemp.State == "Inquisition_toCuracao")
			{
                if (CheckAttribute(pchar, "questTemp.State.Store") || CheckAttribute(pchar, "questTemp.State.Usurer"))
                {
        			...
        			link.l2 = "Пожалуй, нет ещё, монсеньор. Я ещё поработаю над этим...";
        			link.l2.go = "exit";
                }
                else
                {
			...
			if (pchar.questTemp.State == "Inquisition_afterFrancisco")
			{
                ...
                else
                {
        			...
        			link.l1 = "Нет, монсеньор. Я вернул"+ GetSexPhrase("ся","ась") +" не за этим. Сейчас ко мне подошёл некий Франциско де Сан-Агостиньо и соловьиными трелями начал заливать про мораль вашего задания. Он знает, куда я еду, зачем еду. Вы не находите это странным?";
...
//********************************* Задания Инквизиции. Квест №2 **********************************
 		case "Step_S2_1":
			dialog.text = "Я знаю, "+ GetSexPhrase("сын мой","дочь моя") +", знаю. Я ждал тебя. Итак, о твоей миссии.\n"+
                          ...
                          "Души приверженцев секты янсенистов мы освобождаем от нечестивых уз путём сожжения их тел на костре. Но они не ценят этого, не ценят спасения своей души, которые мы им даём...";
		...
		break;
 		case "Step_S2_6":
			...
			link.l1 = "Аминь... Так в чём же будет заключаться моё задание?";
			link.l1.go = "Step_S2_7";
		break;
 		case "Step_S2_7":
			dialog.text = "Вам нужно навестить бежавших негоциантов с определённой целью.";
		...
		break;
 		case "Step_S2_8":
			...
			link.l1 = "С каждого по 50 тысяч, и в случае неповиновения я могу делать с ними всё, что угодно. Я правильно понял"+ GetSexPhrase("","а") +"?";
			link.l1.go = "Step_S2_9";
		break;
 		case "Step_S2_9":
			dialog.text = "Да, "+ GetSexPhrase("сын мой","дочь моя") +", ты всё правильно понял"+ GetSexPhrase("","а") +"...";
			link.l1 = "Но разве 50 тысяч способны искупить грехи янсенистов перед Господом?";
			link.l1.go = "Step_S2_10";
		break;
 		case "Step_S2_10":
			dialog.text = "Нет, конечно. Но мы должны с чего-то начинать. Нам нужно обратить их взор к Господу, этим вы и займётесь. Вам всё понятно, вы готовы к выполнению миссии?";
		...
		break;
 		case "Step_S2_OnlyUsurer":
            ...
            {
                dialog.text = "Э-э-э, задание выполнено, а где же 50 тысяч? Ступай, "+ GetSexPhrase("сын мой","дочь моя") +", и приходи с деньгами для отчёта по заданию.";
		...
		break;
 		case "Step_S2_14":
            ...
            {
    			link.l2 = "Подождите, монсеньор. У меня ещё кое-что есть для вас...";
		...
		break;
 		case "Step_S2_OnlyStore":
            ...
            {
                dialog.text = "Э-э-э, задание выполнено, а где же 100 тысяч? Ступай, "+ GetSexPhrase("сын мой","дочь моя") +", и приходи с деньгами для отчёта по заданию.";
    			link.l1 = "Прошу прощения, монсеньор, забыл"+ GetSexPhrase("","а") +" на корабле. Я мигом...";
    			link.l1.go = "exit";
		...
		break;
 		case "Step_S2_15":
            ...
            if (CheckCharacterItem(pchar, "Bible"))
            {
    			link.l2 = "Подождите, падре. У меня ещё кое-что есть для вас...";
    			link.l2.go = "Step_S2_17";
            }
			pchar.questTemp.State = "Inquisition_backPartGood";
		...
		break;
 		case "Step_S2_20":
			dialog.text = "Святая Богородица, Матерь Божья!!! О такой удаче мы даже и мечтать не могли! Только и думали о том, где появится этот еретический пасквиль и что будем предпринимать в связи с этим.\n"+
                          "Какой груз с плеч долой!.. "+ GetSexPhrase("сын мой","дочь моя") +", в награду за такой поступок я отпускаю тебе все грехи и ещё три авансом!";
		...
		break;
 		case "Step_S2_21":
			dialog.text = "Это ещё не все, "+ GetSexPhrase("сын мой","дочь моя") +". В знак признательности за содеянные заслуги Святая Инквизиция дарует тебе вот этот великолепный защитный доспех, инкрустированный золотом.\n"+
                          "Однако ценность этой кирасы - не в инкрустации. Её истинная ценность - в величайшей надёжности, что тебе и нужно более всего, как я полагаю...";
		...
		break;
 		case "Step_S2_22":
			dialog.text = "Да, да, конечно. Я тоже побегу, отпишу о радостной новости в Севилью!";
