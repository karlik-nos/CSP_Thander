...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
			...
			else
			{
				...
				else
				{
					...
					else
					{
						if (npchar.location == sld.location)
						{
							dialog.text = LinkRandPhrase("Простите, но она находится прямо здесь, " + npchar.quest.seekIdx.where + ". Вам нужно быть внимательным.", "Хм, она ведь здесь, среди нас!", "Если бы вы посмотрели вокруг более внимательно, то наверняка уже увидели бы её...");
						}
						else
						{
		...
		case "SessilDead_3":
			...
			link.l1 = "Вы тоже давно живёте в Городе?";
			link.l1.go = "SessilDead_4";
		break;
		case "SessilDead_4":
			dialog.text = "Не так давно, как жила Сесил, но всё же давно. Механика я видела...";
			link.l1 = "Так вы говорите, что этот Хенрик находится на барке 'Сан Габриэль'?";
			link.l1.go = "SessilDead_5";
		break;
		case "SessilDead_5":
			dialog.text = "Да. Думаю, что адмирал держит Хенрика в изоляции специально, поэтому и отдал барк 'Сан Габриэль' клану 'Каспер'. Всё дело в том, что Механик - необыкновенно талантливый человек. Когда нужно смастерить что-нибудь, ему равных нет. Одно время начал он рассказывать об какой-то опасности, грозящей нам всем. Говорил о том, что мы должны покинуть Город, пока не поздно\n"+
		...
		break;
		case "SeekCrew_3":
			dialog.text = "Я поняла вас. Всё сделаю, как вы сказали.";
