	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			else
			{
				...
				{
					link.l4 = "Ты знаешь, ровно в полночь я был"+ GetSexPhrase("","а") +" в твоем погребе. Адмирал и ещё какой-то тип собирались поговорить, но заметили меня и ушли. Так что я ничего не услышал"+ GetSexPhrase("","а") +".";
		...
		break;
		case "ansewer_6_3":
			dialog.text = "Чёрт! Чёрт!! Чёрт!!! Неужели это возможно?..";
		...
		case "ansewer_12_2":
			...
			link.l1 = "Да, не густо... Но всё равно, спасибо тебе, " + npchar.name + ".";
			link.l1.go = "int_quests";
		break;
		...
		case "TizerCellarLate":
			dialog.text = "Это мне известно, ведь я ждал тебя там битый час! Чёрт возьми, нельзя же быть так"+ GetSexPhrase("им","ой") +" легкомысленн"+ GetSexPhrase("ым","ой") +" в таком серьезном деле!";
		...
		case "TizerCellarOk_6":
			dialog.text = "И ещё нужно не забывать, что адмирал вряд ли будет стоять, слушать тебя и репу чесать в растерянности. Требовать отмены положения Закона, который внесен Каппером и ревностно им исполняется, - большой риск. Мы должны убедить адмирала в своей правоте, а сделать это будет непросто, как мне кажется.";
		...
		case "AfterProtector_2":
			dialog.text = "Дьявол! Это надо же! Ну, теперь тебе конец... Слушай, будь так добр"+ GetSexPhrase("","а") +", не рассказывай, что это я тебя надоумил. Тебя уже всё равно убьют, а я ещё пожить могу...";
		...
		break;
		case "Interception_2":
			dialog.text = "Так вот, будь там сегодня до полуночи, встань возле решетки за колонной и подожди. Только повернись так, чтобы плечи твои не было видно из-за колонны. Возможно, тебе удастся подслушать очень интересный разговор, который повлияет на твоё решение о дальнейших действиях.";
		...
		case "InterceptionBadWork_2":
			...
			link.l1 = "Ну, может ещё не поздно? Давай подождем их следующую встречу.";
		...
		case "NB_3":
			...
			link.l1 = "Чёрт возьми!.. Теперь ты хозяин таверны, как я понимаю.";
			link.l1.go = "NB_4";
		break;
		case "NB_4":
		...
		break;
		case "ADied_3":
			dialog.text = "Значит, они всё-таки вышли из-под контроля. М-да... А за что они её убили?";
