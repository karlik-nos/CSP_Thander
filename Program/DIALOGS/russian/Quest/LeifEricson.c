	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
            if (pchar.questTemp.Ascold == "Ascold_MummyIsLive")
            {
        		...
        		link.l1 = environment.date.year + "-й... Чёрт возьми, что происходит?!!";
        		link.l1.go = "Step_8";
            }
            if (pchar.questTemp.Ascold == "Ascold_BodyChange")
            {
            	dialog.text = "Ну вот, теперь всё в порядке. Ты готов"+ GetSexPhrase("","а") +" выполнить свое предназначение?";
        ...
        case "Step_24":
        	dialog.text = "В общем, да. Только ещё одно ма-а-аленькое дельце, для того, чтобы ты побыстрей возвращал"+ GetSexPhrase("ся","ась") +".\n"+
                          "На время я беру твоё тело, а тебе даю свое. Тоже на время.";
    		link.l1 = "Что?!!!";
