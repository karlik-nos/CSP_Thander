...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
    		...
    		if (npchar.id == "QStranger")
    		{
    			...
    			link.l3 = "Боюсь, положение твоё серьезное. Твой обман, э-э... Раф, был раскрыт. Если ты немедленно не вернешь 30 000 ростовщику, тебя вздернут сегодня же!";
		...
		case "Merchant_0":
        	...
        	dialog.text = "Я не буду спрашивать, для чего это вам понадобилось, и если меня спросят, мне не придётся лгать и выкручиваться. Тем не менее, отчет с меня попросят не ранее чем послезавтра, так что...";
	...
	case "QHStep_3":
		dialog.text = "Да ничего особенно нового... приехал недавно какой-то купец рейсом с Ямайки. Поговаривают, привез очень ценный груз. Если ничего не путаю, он вроде старый знакомый полковника Бишопа. Разместился в одном из домов города, собирается пробыть тут ещё день-два.";
    ...
    case "FStep_5":
        ...
        dialog.text = "Нет? Нет рыбы?! Надо знать места, недотепа! Я нашёл уникальное место... сеть наполняется до отказа за ночь - Господь свидетель!";
