...
/*
    ...
    */
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
            ...
            if (npchar.id == "Bridgetown_Mayor" )
            {
                ...
                if( Pchar.questTemp.CapBloodLine.stat == "PrepareToEscape3" )
                {
            		...
            		link.l1 = "Добрый день, губернатор. Я принес ещё профилактического экстракта для вашей супруги и хотел бы иметь возможность осмотреть её.";
            		link.l1.go = "SStep_9";
            		break;
                }
    		...
    		if (npchar.id == "Den")
    		{
    			...
    			{
            			link.l2 = "У миссис Стид приступ мигрени, меня срочно вызвали к ней. Я осмотрел её и вынужден был отправиться к вам в столь поздний час за необходимым лекарством. Не сумев вас разыскать, я позволил себе взять лекарство без разрешения - ситуация крайне серьезная. Прошу прощения.";
            			link.l2.go = "DStep_1";
    			}
    		...
    		if (npchar.id == "Spain_spy" )
    		{
    		 ...
    		 if(npchar.id == "Ogl")
    		 {
                if(Pchar.questTemp.CapBloodLine.statcrew == "find")
                {
                    ...
                    link.l1 = "Как насчет дельца, Огл? Есть ещё порох в пороховницах?";
		...
		case "BStep_1":
        	dialog.text = "Если ты будешь совать свой нос в мои дела, мне придётся кое-что предпринять, красавец, чтобы ты не лодырничал и не злоупотреблял предоставленной тебе свободой. Не забывай, что ты осужденный бунтовщик!";
        ...
        // --> Стид
 		...
 		case "SStep_0":
            ...
            dialog.text = "Как бы там ни было, сейчас главное - моя жена. У нее опять приступ мигрени, она перевозбуждена и отвергает все попытки уложить её в постель!";
        ...
        case "MSStep_0":
        	...
        	link.l1 = "Похоже, никак не обойтись без лекарств, но аптека давно закрыта. Мне придётся пойти к мистеру Дэну домой, чтобы получить все необходимое. Попейте теплой воды и ждите меня, я вернусь так скоро, как только смогу!";
            link.l1.go = "Exit";
        ...
        case "MSStep_7":
        	...
        	link.l1 = "Вот, я принес вам ещё немного чудодейственного экстракта, что и было изначальной целью моего визита. Всего наилучшего.";
            TakeItemFromCharacter(Pchar, "migraine_potion");
        ...
        case "WStep_12":
        	...
        	link.l1 = "Это будет затруднительно, но выбора у меня нет. Что насчет шлюпа? Никто из рабов не сможет просто прийти на верфь и прикупить себе шлюп, как ни в чём не бывало.";
            link.l1.go = "WStep_13";
        ...
        case "PStep_17":
        	...
        	link.l1 = "Думаю, у нас ещё будет время поразмыслить над этим. А сейчас нельзя терять времени, те, кто остался на берегу - не слепые и не умерли и скоро поймут, что с  кораблем что-то неладно. Нужно выбрать капитана и подготовиться к обороне.";
            link.l1.go = "PStep_18";
...
//            SetCharacterGoods(Pchar,GOOD_RUM,40);//600);
        ...
        case "HStep_6":
            ...
            dialog.text = "Я готов заплатить тебе две тысячи за кое-какую информацию. Дело в том, что Арнольд Саммерлайт - мой конкурент - нашёл какое-то место... очень рыбное место. Он разоряет меня! Узнай любой ценой, где он добывает свой улов, и я с радостью расстанусь с двумя тысячами. Идет?";
        	link.l1 = "Идет.";
            link.l1.go = "HStep_7";
        ...
        case "HTStep_12":
        	...
        	link.l1 = "Пока ещё нет, я сообщу, когда придет время.";
            link.l1.go = "Exit";
            NextDiag.TempNode = "HTStep_12";
