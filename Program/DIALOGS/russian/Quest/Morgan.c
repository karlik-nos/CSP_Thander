        ...
        break;
 	 	case "Step_H3_6":
            if (CheckAttribute(pchar, "questTemp.Talks.Tavern") && CheckAttribute(pchar, "questTemp.Talks.Store") && CheckAttribute(pchar, "questTemp.Talks.Shipyard") && CheckAttribute(pchar, "questTemp.Talks.Usurer") && CheckAttribute(pchar, "questTemp.Talks.Brothel"))
 	 	    {
    			dialog.text = "Ну что же, я полагаю, все ясно. Такой суммой Пикардиец не располагал ранее и не получил в результате проведенной совместно операции. Учитывая мои подозрения, ему придётся серьезно попотеть, дабы обосновать такие траты... Я благодарю вас.";
