...
//			LAi_ActorGoToLocatorFunc(NPChar, "reload", "reload6_back", "Andre_Abel_Quest_Delete_Andre", 15);
		...
		case "Andre_Abel_In_Tavern_7":
			dialog.text = "Выручи, а?.. За это я дам тебе торговую лицензию на предъявителя. Очень нужная вещь, ты сам"+ GetSexPhrase("","а") +" знаешь. И ещё коллекцию драгоценностей. Ну что, возьмешься?";
		...
		case "Andre_Abel_In_Tavern_Continue_Quest":
			...
			{
				link.l1 = LinkRandPhrase("Нет ещё.", "Да не волнуйся ты так. Всё у нас получится...", "Андре, расслабься... отдыхай, развлекайся, всё равно от тебя сейчас ничего не зависит.");
				link.l1.go = "exit";
