...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "PKInMarigo_4":
			...
			link.l1 = "Хорошо. Я знаю, что вы являетесь представителем Голландской Вест-Индской компании в Карибском море. Знаю также, что вы ведете дела с генерал-губернатором Стэвезантом. И ещё я хочу знать, какого черта вам не угодили местные бродяги.";
		...
		case "PGG_cabin_1":
			...
			link.l2 = "Самоуверенность, да ещё и жадность. Нет, мы с тобой не найдем общего языка. Защищайся!";
			link.l2.go = "PGG_cabin_fight";
		break;
		case "PGG_cabin_2":
		...
		break;
		case "NewDeckSharp_1":
			dialog.text = "Да всё путем, "+ GetSexPhrase("приятель","дорогуша") +", жаловаться грех\nНу, рассказывай, зачем пожаловал"+ GetSexPhrase("","а") +" на этот раз?";
		...
		case "NewDeckSharp_noMoney":
			if ((sti(pchar.questTemp.Sharp.price)*10) < sti(pchar.money))
			{
				if (pchar.location == "Deck_Near_Ship")
				{
					...
					link.l1 = ""+ GetSexPhrase("Бартоломью, ты чего завелся?! Скрипишь, как рангоут... Я к тебе не клотик пришёл швабрить и не якорь точить. Так что ты аккуратней в выражениях. Помни, с кем разговариваешь","Бартоломью, ты чего раздухарился? Ты это, аккуратнее в выражениях, я тебе не какая-нибудь там девка бордельная, не забывай этого") +"...";
				}
				else
				{
		...
		case "DeckSharp_over":
			dialog.text = "Мы всё обсудили, "+ GetSexPhrase("приятель","подруга") +". На твоем месте я бы не стал терять времени.";
		...
		case "PortmansCap_3":
			...
			{
				dialog.text = "Новый журнал я ещё не завел, как раз собирался, поэтому заплачу максимум из того, что могу себе позволить. " + FindRussianMoneyString(sti(npchar.quest.money)) + " и немного моих личных драгоценностей.";
			...
			else
			{
				if (sti(npchar.quest.stepsQty) < 5)
				{
					...
					dialog.text = "Хм, ты знаешь, я уже завел новый журнал. Тем не менее, старый журнал мне нужен ещё, необходимо всё перенести в новый. Так что я могу забрать его за " + FindRussianMoneyString(sti(npchar.quest.money)) + ". и немного моих личных драгоценностей.";
		...
		case "PortmansCap_inDeck_2":
			...
			else
			{
				...
				{
					npchar.quest.money = makeint(sti(npchar.quest.money) / sti(npchar.quest.stepsQty)); //уменьшаем вознаграждение
					dialog.text = "Чёрт, так, значит, вот где я его потерял!.. Спасибо тебе, конечно, но дело в том, что ты долго меня искал"+ GetSexPhrase("","а") +", и я уже завел новый. Однако, перенести кое-что мне оттуда нужно, поэтому я заберу журнал у тебя. Но много заплатить не могу...";
					link.l1 = "А сколько можешь?";
					link.l1.go = "PortmansCap_inDeck_3";
				}
				else
				{
					dialog.text = "Чёрт, так, значит, вот где я его потерял!.. Спасибо тебе, конечно, но дело в том, что ты долго меня искал"+ GetSexPhrase("","а") +", и я уже завел новый. Старые записи перенес по памяти, так что мне он не нужен.";
		...
		case "SeekCap_inDeck_1":
			dialog.text = "Ничего не продается, и новостей у меня нет. И вообще, честно говоря, твоё присутствие у меня на палубе нежелательно. Надеюсь, ясно выразился?";
		...
		case "SCQ_RapeWife":
			dialog.text = "Моё относительное бездействие привело к печальным результатам. Этот мерзавец похитил мою жену и увез её на своем корабле в море. Я прошу вас найти этого негодяя!";
		...
		case "SCQ_manResult":
			...
			switch (npchar.quest.SeekCap)
			{
				...
				case "manFriendover":
					...
					link.l1 = "Наш"+ GetSexPhrase("ел","ла") +" и рассказал"+ GetSexPhrase("","а") +", что вы его ждете здесь. Так что всё вышло удачно.";
		...
		case "SCQ_woman_1":
			...
			{
				case "0":
					dialog.text = "Мой муж занимается коммерцией - доставляет грузы торговцам по всей округе. Так вот, он ушёл в море уже больше трех месяцев назад, и до сих пор не вернулся!";
		...
		case "SCQ_Revenge_3":
			...
			link.l1 = "Хм... "+ GetSexPhrase("а","да, мужчины на это способны. Но,") +" может, вы остынете? Ну, пройдет день-другой, и всё изменится...";
		...
		break;
		case "FriendCap_2":
			dialog.text = "Ха, ещё бы! А где он живет?";
		...
		case "FriendCap_inDeck_2":
			dialog.text = "О-о-о, нашёлся всё-таки! Очень хорошо... Вы меня извините за грубый прием, я, знаете ли, грешным делом подумал, что вы пират.";
