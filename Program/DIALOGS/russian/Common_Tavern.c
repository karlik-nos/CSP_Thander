...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			if(NPChar.quest.meeting == "0")
			{
				...
				{
					dialog.Text = "Ба, вот и ещё "+ GetSexPhrase("один лихой капитан","одна отважная морячка") +" на огонек пожаловал"+ GetSexPhrase("","а") +"! Располагайтесь, судар"+ GetSexPhrase("ь","ыня") +". Меня зовут Лаки Росин, и я всегда к вашим услугам. Погодка сегодня радует, не правда ли? Желаете рому?";
			...
			else
			{
				...
				if(CheckAttribute(PChar, "GenQuest.ChurchQuest_1.NeedToDialogWithBarmen") && PChar.location == PChar.GenQuest.ChurchQuest_1.CurPortManColony + "_tavern")
		        {
		            if(CheckAttribute(PChar, "GenQuest.ChurchQuest_1.NoMoneyToBarmen"))
		            {
						...
						{
							link.l1 = "Нет, ещё не принес"+ GetSexPhrase("","ла") +"...";
							link.l1.go = "exit";
						}
		            }
		            else
	...
	////////////////////////////////////////////////////////=============================================///////////////////////////////////////////////////////
		...
		case "room":
			...
			{
				dialog.text = "Вы заплатили мне за неделю вперед, я не могу брать у вас ещё деньги. Комната ваша, идите и отдыхайте...";
		...
		case "TStep_2":
            ...
            {
    			link.l1 = "Я разве не упомянул, что у меня для знающего человека найдется премия в размере... ну, скажем, трёх сотен пиастров? Да и вспомните, как я лечил вашу дочь, не жалея своих сил и времени.";
    			link.l1.go = "TStep_4";
            }
		break;
