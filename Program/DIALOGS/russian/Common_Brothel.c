...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
        ...
        case "Hostess_Choice":
			dialog.text = "Я всегда рада, когда между девочками и клиентами возникают теплые чувства... Назови мне её имя.";
			Link.l1.edit = 9;
			Link.l1 = "";
		...
		break;
        case "Horse_1":
			dialog.text = "Она в своем кабинете, вы можете пройти к ней отсюда через дверь, противоположную выходу на улицу, либо через улицу с другой стороны дома. Её зовут " + characters[GetCharacterIndex(npchar.city + "_Hostess")].name + ".";
		...
		break;
        case "Horse_3":
			dialog.text = characters[GetCharacterIndex(npchar.city + "_Hostess")].name + " все оформляет в своем кабинете. Иди к ней"+ GetSexPhrase(", мой герой,","") +" и назови моё имя - " + npchar.name + ". Я буду тебя ждать...";
		...
		case "Horse_ReasonToFast_2":
			...
			dialog.text = "Да был перед тобой караульный старшина. Он вообще-то к нам не ходит, а тут принесла нелегкая. Ещё и меня выбрал... Возился-возился, потом обозвал меня неумехой и заторопился в бухту какую-то. Так быстро убежал, чуть было портки не оставил, ха-ха-ха...";
...
//********************************* Диалоги девки из борделя ФФ **********************************
		...
		case "Step_Hol8_6":
			...
			link.l1 = "Хм, раз она продается, то я хотел"+ GetSexPhrase("","а") +" бы её приобрести. Как я могу найти этого корсара?";
