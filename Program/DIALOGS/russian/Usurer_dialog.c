...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			{
				link.l8 = "" + npchar.name + ", твоё задание выполнено. Мерзавец Гонтьер мертв, как и вся его команда.";
		...
		break;
		case "StepPL5Four_4":
			dialog.text = "Да, причем здесь Эйвори? Грабитель, конечно! Слушайте, что вы меня всё время перебиваете?!";
		...
		break;
		case "StepPL5Four_6":
			dialog.text = "Пятьдесят тысяч! Если вы мне всё это принесете, ваша награда составит пять тысяч.";
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		break;
		case "usurersJewel_2":
			dialog.text = "Эх, если бы всё было так просто! Дело в том, что этот " + npchar.quest.usurersJewel.type + " уникален в своем роде. Этот камень передал мне заказчик для изготовления перстня. Подобрать что-нибудь подобное по размеру и форме я не имею возможности...";
		...
		case "CapBloodUsurer_5":
			dialog.text = "Сегодня рано утром у меня взял заем человек из города. Дело в том, что... он меня обманул. Он назвался именем Рафаэля Гиннеса, который частенько ко мне заходит, и, признаться, немного на него похож. Я дал ему заем в размере 30 000 пиастров, но потом зашёл Раф и...";
		...
		case "Checkslaves":
			...
			dialog.Text = "Ну что, удалось вам добыть уговоренную партию 'чёрного дерева'?";
            if (amount < 0)
			{
		...
		case "Takeslaves_6":
			dialog.Text = "С учётом того, что я даю полную информацию о местонахождении груза, избавляя вас от необходимости бороздить море в поисках добычи, оплата будет, как вы сами понимаете, несколько ниже. По 200 пиастров за каждого раба. Но это всё равно огромная сумма - порядка трехсот тысяч, если всё пройдёт успешно.";
		...
		case "node_hanter_1":
			dialog.text = "Это ещё не всё. Я никому не позволяю водить себя за нос! И за свое вероломство ты поплатишься - я обеспечу тебе проблемы, не сомневайся. А теперь проваливай!";
		...
		case "Takeslaves_7_win":
			dialog.text = "Оно принесет вам целое состояние... Да, ещё такой вопрос, " + pchar.name + ": во время абордажа вы ничего такого в каюте галеона не находили?";
		...
		case "Slaveshore_5":
			...
			link.l1 = "Ладно, вижу, что не врешь. На твоё счастье капитан пинаса перед смертью проболтался, что наши дела раскрыты. Если бы не эта его фраза, я бы уже прикончил"+ GetSexPhrase("","а") +" тебя... Но ты же говорил, чёртов денежный мешок, что у тебя здесь на острове всё схвачено!";
		...
		case "Slaveshore_7": //выбор острова, куда бригантина пойдет
			...
			link.l1 = "Большего мне не остается. Ну что же, молитесь, и за себя, и за меня, и надейтесь, что я её догоню. А когда вернусь - мы ещё продолжим наш разговор. И не думайте, что я всё это вот так оставлю!";
		...
		case "Brig_win_2":
			dialog.text = "Безусловно, " + pchar.name + ". Более того, я обязан вам очень многим. И весьма рассчитываю на продолжение нашего сотрудничества. Но прошу вас войти в моё положение - я понес огромные траты - лояльность губернатора и коменданта стоит недешево. Кроме того, пришлось замять убийство патрульного наряда в моей резиденции...";
		...
		case "Wingalleon":
			...
			{
			dialog.text = "Нашли галеон? А где рабы? Там должно было быть не меньше трех тысяч рабов! Чёрт возьми, кругом одни негодяи и предатели! Вот от кого-кого, а от вас я этого не ожидал... Вы были моей последней надеждой\nНу, раз так... Клянусь, я потрачу остатки своего состояния и пойду на паперть, но сделаю так, что за вами будет гоняться весь флот " + NationNameGenitive(sti(npchar.nation)) + "! Убирайтесь с глаз моих долой!";
				link.l1 = "Эм-м...";
		...
		case "FindRat_1":
			AddMoneyToCharacter(pchar, makeint(pchar.questTemp.Slavetrader.iSlaveMoney));
			dialog.text = "Вы всегда можете верить моему слову, " + pchar.name + ". Я ещё ни разу не дал вам повода усомниться в этом. Так будет и впредь. Честность партнеров друг с другом - это фундамент успеха в любом совместном бизнесе\nТеперь об индейских артефактах. Показывайте, что у вас там есть, а то я, как истовый коллекционер, уже весь извелся.";
		...
		case "FindRat_2":
			...
			{
				dialog.text = "Вот! Замечательно! Базальтовая голова Ольмеков! Это редкий артефакт. Я давно хотел иметь её в своей коллекции. Предлагаю вам за нее 30 000, или обмен на статуэтку Тлалока, у меня таких две.";
		...
		case "PF_check":
			...
			{
				dialog.text = "Вот! Я её сразу не заметил! Меня интересует вот эта фигурка индейского юноши. Очень занятный предмет, скажу я вам... Я готов заплатить за нее 20 000 пиастров, либо обменять на амулет Шиукоатля.";
				link.l1 = "Я возьму деньги.";
				link.l1.go = "PF_money";
		...
		case "FindRat_4":
			dialog.text = "Я говорю совершенно серьезно. И именно потому, что вы обладаете столь выдающимися способностями, и потому, что я вам полностью доверяю, я прошу вас выполнить моё очередное поручение.";
		...
		case "Havana_fort_1":
			dialog.text = "В течение последнего месяца в Гавану со всего Мейна, мелкими партиями, свозились порабощенные индейцы, кроме того, туда прибыли из Африки и разгрузились несколько галеонов с 'чёрной слоновой костью'. В данный момент в Гаване, за высокими стенами форта, находится не меньше пяти тысяч рабов.";
			link.l1 = "Чувствую я, что речь пойдет ни больше, ни меньше, как о штурме Гаваны.";
			link.l1.go = "Havana_fort_2";
		break;

		case "Havana_fort_2":
			dialog.text = "Вы всё правильно понимаете, " + pchar.name + ". Именно это я и хочу вам поручить. Вам и только вам. Не думаю, что кто-либо из моих других агентов сможет провести столь серьезную операцию\nВы ведь, безусловно, возьметесь за это дело?";
		...
		case "Havana_Fort_yes_3":
			dialog.text = "Вот видите... " + pchar.name + ", вы мой лучший агент, и я рассчитываю вести с вами бизнес в дальнейшем. Может, хватит уже постоянно высказывать мне свое недоверие, да ещё в такой... резкой форме?";
		...
		case "Usurer_Escape_4":
			...
			link.l1 = "Вот проклятье! Это же надо было - довериться такому мерзавцу! От него же за версту несло подлостью! А я, простофиля, поверил"+ GetSexPhrase("","а") +", что мы действительно партнеры! Но он ещё здорово пожалеет... Ладно, " + npchar.name + ", прошу меня извинить за грубость... До свидания.";
