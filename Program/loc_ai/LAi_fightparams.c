...
if (isBlocked && blockSave)
	{
		if (fencing_type == "FencingHeavy" && attackType == "break")
		{
   ...
   if (critchance > 0 && rand(100 - GetCharacterSPECIALSimple(attack, SPECIAL_L)) <= critchance) critical = 1.0;
	...
	if (!blockSave && dmg>0)
	{
			if(cirign || !CheckAttribute(enemy,"cirassId"))
			{
				if(HasSubStr(attack.equip.blade, "blade201") && rand(100) == 10)
				{
					Log_Info("Вы вызвали кровотечение.");
					PlaySound("interface\Krovotok_"+rand(4)+".wav");
					MakeBloodingAttack(enemy, attack, coeff);
				...
				if(CheckAttribute(enemy, "model.animation") && CheckAttribute(enemy, "sex")) MakeSwiftAttack(enemy, attack, coeff);
				}
			}
	}
	//<---Функционал среднего оружия - Gregg
